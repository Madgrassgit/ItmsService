 ...
 * @date 2010-6-21
 */
public class ServiceDoneChecker extends NmgBaseChecker
{

	// 日志记录对象
	private static Logger logger = LoggerFactory.getLogger(ServiceDoneChecker.class);
	// 业务类型
	 ...
	 *            接口调用入参，xml字符串
	 */
	public ServiceDoneChecker(String inXml)
	{
		callXml = inXml;
	}
	...
	public boolean check()
	{
		...
		try
		{
			...
			if ((!"nx_dx".equals(Global.G_instArea)) && (!"nmg_dx".equals(Global.G_instArea)))
			{
	...
	private boolean serviceTypeCheck()
	{
		...
		if (10 != serviceType && 11 != serviceType && 14 != serviceType
				&& 0 != serviceType)
		{
			result = 1002;
			resultDesc = "业务类型非法";
