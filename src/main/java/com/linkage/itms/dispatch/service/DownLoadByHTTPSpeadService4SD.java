...
import com.linkage.itms.dispatch.obj.DownLoadByHTTPSpeadChecker4SD;
 ...
 * @version 1.0
 * @since 2018年9月27日
 * @山东电信HTTP下载宽带上网测速对外接口需求
 */
public class DownLoadByHTTPSpeadService4SD implements IService
{

	private static Logger logger = LoggerFactory
			.getLogger(DownLoadByHTTPSpeadService4SD.class);

	@Override
	public String work(String inXml)
	{
		logger.warn("DownLoadByHTTPSpeadService4SD==>inXml({})", inXml);
		DownLoadByHTTPSpeadChecker4SD checker = new DownLoadByHTTPSpeadChecker4SD(inXml);
		checker.setIsInsert("0");
		if (false == checker.check())
		{
			logger.warn(
					"servicename[DownLoadByHTTPSpeadService4SD]cmdId[{}]userInfo[{}]验证未通过，返回：{}",
		...
		if (null == deviceInfoList || deviceInfoList.size() == 0)
		{
			...
			logger.warn(
					"servicename[DownLoadByHTTPSpeadService4SD]cmdId[{}]userInfo[{}]没有查到设备",
					new Object[] { checker.getCmdId(), checker.getUserInfo() });
			return checker.getReturnXml();
		}
		if (deviceInfoList.size() > 1)
		{
			...
			logger.warn(
					"servicename[DownLoadByHTTPSpeadService4SD]cmdId[{}]userInfo[{}]查到多组设备，请输入更多位设备序列号进行查询",
					new Object[] { checker.getCmdId(), checker.getUserInfo() });
			return checker.getReturnXml();
		}
		...
		if (null == deviceInfoMap || deviceInfoMap.isEmpty())
		{
			...
			logger.warn(
					"servicename[DownLoadByHTTPSpeadService4SD]cmdId[{}]userInfo[{}]查无此设备",
					new Object[] { checker.getCmdId(), checker.getUserInfo() });
			return checker.getReturnXml();
		}
		//解决用户业务表根据宽带账号查询到有数据但是没有设备
		...
		// 设备存在
		// 速率查询
		if (!StringUtil.IsEmpty(checker.getSpeed()))
		{
			...
			if (null == speadMap || speadMap.size() == 0)
			{
				...
				logger.warn(
						"servicename[DownLoadByHTTPSpeadService4SD]cmdId[{}]userInfo[{}]宽带速率非法",
						new Object[] { checker.getCmdId(), checker.getUserInfo() });
				return checker.getReturnXml();
			}
		...
		/*else if (!"xj_dx".equals(Global.G_instArea) && !"nx_dx".equals(Global.G_instArea)&& !"sd_dx".equals(Global.G_instArea))
		{
						...
						.getTestPassword())))
		{
			...
			logger.warn(
					"servicename[DownLoadByHTTPSpeadService4SD]cmdId[{}]userInfo[{}]桥接模式时测速账号测速密码不可为空",
					new Object[] { checker.getCmdId(), checker.getUserInfo() });
			return checker.getReturnXml();
		}
		...
		if (StringUtil.getIntegerValue(checker.getSpeed()) > 100)
		{
			...
			if (null == gbBroadBandMap || gbBroadBandMap.isEmpty())
			{
				...
				logger.warn(
						"servicename[DownLoadByHTTPSpeadService4SD]cmdId[{}]userInfo[{}]ONU不支持千兆测速功能",
						new Object[] { checker.getCmdId(), checker.getUserInfo() });
				return checker.getReturnXml();
			}
			else
			{
				...
				if ("1".equals(isGb))
				{
					// 支持千兆宽带时，继续测速
					logger.warn(
							"servicename[DownLoadByHTTPSpeadService4SD]cmdId[{}]userInfo[{}]ONU支持千兆测速功能",
							new Object[] { checker.getCmdId(), checker.getUserInfo() });
				}
				else
				{
		...
		if (null == netAccounts || netAccounts.size() == 0)
		{
			...
			logger.warn(
					"servicename[DownLoadByHTTPSpeadService4SD]cmdId[{}]userInfo[{}]宽带账号为空",
					new Object[] { checker.getCmdId(), checker.getUserInfo() });
			return checker.getReturnXml();
		}
		...
		if (flag == 1)
		{
			...
			if (StringUtil.IsEmpty(Interface))
			{
				...
				logger.warn(
						"servicename[DownLoadByHTTPSpeadService4SD]cmdId[{}]userInfo[{}]wan通道获取失败",
						new Object[] { checker.getCmdId(), checker.getUserInfo() });
				return checker.getReturnXml();
			}
			/*if ("jx_dx".equals(Global.G_instArea))
			{
				...
				logger.warn("DownLoadByHTTPSpeadService4SD==>downURL is:" + urlList[i]);
				logger.warn("DownLoadByHTTPSpeadService4SD==>start device_id is:"
						+ deviceId);
			}*/
			// 开始PPPOE拨测
	...
	public String downLoadByHTTPSpead(String gw_type, String deviceId,
			DownLoadByHTTPSpeadChecker4SD checker, String Interface)
	{
		...
		if (devRPCRep == null || devRPCRep.size() == 0)
		{
			...
			logger.error(
					"servicename[DownLoadByHTTPSpeadService4SD]cmdId[{}]userInfo[{}]设备未知错误",
					new Object[] { checker.getCmdId(), checker.getUserInfo() });
			return checker.getReturnXml();
		}
		...
		else
		{
			...
			if (stat != 1)
			{
				...
				checker.setResultDesc(errMessage);
				logger.error(
						"servicename[DownLoadByHTTPSpeadService4SD]cmdId[{}]userInfo[{}]{}",
						new Object[] { checker.getCmdId(), checker.getUserInfo(),
								errMessage });
				return checker.getReturnXml();
			}
			else
			{
						...
						|| devRPCRep.get(0).getRpcList().size() == 0)
				{
					...
					checker.setResultDesc(errMessage);
					logger.error(
							"servicename[DownLoadByHTTPSpeadService4SD]cmdId[{}]userInfo[{}]终端不支持",
							new Object[] { checker.getCmdId(), checker.getUserInfo() });
					return checker.getReturnXml();
				}
				else
						...
						for (int k = 0; k < rpcList.size(); k++)
						{
									...
									.getRpcName()))
							{
								...
								if (resp == null || "".equals(resp))
								{
									...
									logger.error(
											"servicename[DownLoadByHTTPSpeadService4SD]cmdId[{}]userInfo[{}]系统内部错误，无返回值",
				...
				if (downByHTTPMap == null)
				{
					...
					checker.setResultDesc("终端设备不支持，无法测速！");
					logger.error(
							"servicename[DownLoadByHTTPSpeadService4SD]cmdId[{}]userInfo[{}]返回值为空，HTTP下载仿真失败",
							new Object[] { checker.getCmdId(), checker.getUserInfo() });
					return checker.getReturnXml();
				}
				else
				{
					...
					if("0".equals(receiveByte) || receiveByte.isEmpty())
					{
						...
						checker.setResultDesc("终端设备不支持，无法测速！");
						logger.error(
								"servicename[DownLoadByHTTPSpeadService4SD]cmdId[{}]userInfo[{}]设备接受字节数异常",
								new Object[] { checker.getCmdId(), checker.getUserInfo() });
						return checker.getReturnXml();
					}
					...
					if(timeInterval <= 0)
					{
						...
						checker.setResultDesc("终端设备不支持，无法测速！");
						logger.error(
								"servicename[DownLoadByHTTPSpeadService4SD]cmdId[{}]userInfo[{}]设备日期节点值异常",
								new Object[] { checker.getCmdId(), checker.getUserInfo() });
						return checker.getReturnXml();
					}
					...
					// "DownLoadByHTTPSpeadService4SD", checker.getUsername());
