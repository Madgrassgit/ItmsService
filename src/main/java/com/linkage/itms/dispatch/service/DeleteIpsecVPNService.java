...
import com.linkage.itms.dispatch.obj.DeleteIpsecVPNChecker;
 ...
 */
public class DeleteIpsecVPNService implements IService {

	private static final Logger logger = LoggerFactory.getLogger(DeleteIpsecVPNService.class);

	@Override
	public String work(String inParam) {
		logger.warn("DeleteIpsecVPNService==>inParam:" + inParam);
		DeleteIpsecVPNChecker checker = new DeleteIpsecVPNChecker(inParam);
		if (false == checker.check()) {
			...
			logger.warn("DeleteIpsecVPNService==>retParam={}", checker.getReturnXml());
		...
		if (null == userInfoList || userInfoList.isEmpty()) {
			logger.warn("servicename[DeleteIpsecVPNService]cmdId[{}]userinfo[{}]查无此用户",
...
//		checker.setLoidPrev("");
		...
		//删除工单
		int res = ipsecUserDeviceDAO.deleteSheet(checker);
		if(res<0){
			logger.warn("servicename[DeleteIpsecVPNService]cmdId[{}]userinfo[{}]UserId[{}]处理结束，删除工单失败",
					new Object[] { checker.getCmdId(), checker.getUserInfo(), checker.getUserId()});
			checker.setResult(1000);
			checker.setResultDesc("删除工单失败");
			return checker.getReturnXml();
		}else{
			logger.warn("servicename[DeleteIpsecVPNService]cmdId[{}]userinfo[{}]UserId[{}]处理结束，删除工单成功",
					new Object[] { checker.getCmdId(), checker.getUserInfo(), checker.getUserId()});
		}
//		if (false == CheckStrategyUtil.chechStrategy(checker.getDeviceId())) {
//			logger.warn("servicename[DeleteIpsecVPNService]cmdId[{}]userinfo[{}]设备繁忙或者业务正在下发，请稍候重试",
//					new Object[] { checker.getCmdId(), checker.getUserInfo() });
//			checker.setResult(1009);
//			checker.setResultDesc("设备繁忙或者业务正在下发，请稍候重试");
//		} else {
			if (true != CreateObjectFactory.createPreProcess()
					.processDeviceStrategy(new String[]{checker.getDeviceId()},"2703",new String[]{"27", userId + "", checker.getUserInfo()})) {
				logger.warn("servicename[DeleteIpsecVPNService]cmdId[{}]userinfo[{}]设备[{}]业务下发，调用配置模块失败",
						new Object[] { checker.getCmdId(), checker.getUserInfo(), checker.getDeviceId() });
				checker.setResult(1000);
				checker.setResultDesc("未知错误，请稍后重试");
			}
//		}
		logger.warn("servicename[DeleteIpsecVPNService]cmdId[{}]userinfo[{}]处理结束，返回响应信息:[{}]",
