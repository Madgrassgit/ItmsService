...
public class PingDiagnostic implements IService {
	...
	public String work(String inXml)
	{
		...
		String isWebInvoke=checker.getIsWebInvoke();
		if(!"true".equals(isWebInvoke)) { //判断是否从web页面调用
			
		// chenxj6 xj
		if ("xj_dx".equals(Global.G_instArea) || "nx_dx".equals(Global.G_instArea) || "jl_dx".equals(Global.G_instArea) || "jl_lt".equals(Global.G_instArea)
				|| "cq_dx".equals(Global.G_instArea) || ("jx_dx".equals(Global.G_instArea) && 1 == checker.getSearchType())) {
			String deviceId = "";
			...
			if (checker.getUserInfoType() == 1) {
				...
				userMapList = dao.queryDeviceByDevSN(checker.getUserInfo());
			} else if (checker.getUserInfoType() == 7) {
				userMapList = dao.queryUserByCustomerId(checker.getUserInfo());
			}
			...
			// update on 2020/5/26 by lm : jl_lt 遇到多用户情况只需取最新的记录即可
			if (userMapList.size() > 1 && !"jl_lt".equals(Global.G_instArea)) {
			...
			// 设备存在
				...
				// ping诊断
				checker.setDevSn(deviceInfoMap.get("device_serialnumber"));
				checker.setOui(deviceInfoMap.get("oui"));
			...
			Map<String, String> deviceInfoMap = deviceInfoDAO.queryDevInfo(checker.getDevSn(),
					checker.getOui());
			if("jx_dx".equals(Global.G_instArea) && 2 == checker.getSearchType()){
				// 江西处理
				// oui-串码
				...
				if (null == devInfoMapList || devInfoMapList.isEmpty()) {
					logger.warn("servicename[PingDiagnostic]cmdId[{}]devSn[{}]查无此设备",
				...
				if (devInfoMapList.size() > 1) {
					logger.warn("servicename[PingDiagnostic]cmdId[{}]devSn[{}]查询到多台设备",
							new Object[] { checker.getCmdId(), checker.getDevSn()});
					checker.setResult(1006);
					checker.setResultDesc("查到多台设备,请输入更多位序列号或完整序列号进行查询");
					return checker.getReturnXml();
				}
				deviceInfoMap = devInfoMapList.get(0);
			}
			// 设备不存在
			if (null == deviceInfoMap || deviceInfoMap.isEmpty()) {
			...
			// 根据设备序列号，厂商OUI检索设备
			Map<String, String> deviceInfoMap = deviceInfoDAO.queryDevInfo(checker.getDevSn(), checker.getOui());
	...
	public String PingList(String gw_type, String device_id, PingDiagnosticChecker checker) {
		//得到设备类型
//		if (Interface.startsWith("InternetGatewayDevice.LANDevice.1.LANEthernetInterfaceConfig"))
//		{
...
//				String vlanId = "41";
		...
		else if("jl_dx".equals(Global.G_instArea) ||"cq_dx".equals(Global.G_instArea) || "jx_dx".equals(Global.G_instArea) || "jl_lt".equals(Global.G_instArea)){
			waninterface = gatherWanPath(device_id, checker);
			logger.warn("[PingList]|deviceId:{},get waninterface:{}",device_id,waninterface);
		...
		if(!"true".equals(isWebInvoke)) {
			if ("xj_dx".equals(Global.G_instArea) || "nx_dx".equals(Global.G_instArea)|| "jl_dx".equals(Global.G_instArea)|| "cq_dx".equals(Global.G_instArea)
					|| "jx_dx".equals(Global.G_instArea) || "jl_lt".equals(Global.G_instArea)) {
...
//							}
	 ...
	 * @return String
	 */
	private String gatherWanPath(String deviceId, PingDiagnosticChecker checker) {
		...
		if("jl_lt".equals(Global.G_instArea)){
            wanServiceList = ".X_CU_ServiceList";
		}
				...
				|| wanConnPathsList.isEmpty())
		{
			...
			logger.warn("gather the jList:{}",jList);
        ...
        logger.warn("[PingDiagnostic] [{}],paramNameList:{}", deviceId,paramNameList);
		if (serviceListList.size() == 0 || serviceListList.isEmpty())
		{
			if(!"jl_lt".equals(Global.G_instArea)){
				logger.warn("[PingDiagnostic] [{}]不存在WANIP下的X_CT-COM_ServiceList节点，返回", deviceId);
			}else {
				logger.warn("[PingDiagnostic] [{}]不存在WANIP下的X_CU_ServiceList节点，返回", deviceId);
			}
			...
			for (Map.Entry<String, String> entry : paramValueMap.entrySet())
			{
				...
				if (paramName.indexOf(wanServiceList) >= 0)
				{
					if (!StringUtil.IsEmpty(entry.getValue())
						...
						String res = "";
						if(!"jl_lt".equals(Global.G_instArea)){
							res=entry.getKey().substring(0, entry.getKey().indexOf("X_CT-COM_ServiceList"));
						}else {
							res=entry.getKey().substring(0, entry.getKey().indexOf("X_CU_ServiceList"));
						}
