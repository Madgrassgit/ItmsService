...
import com.linkage.itms.dispatch.obj.QueryDevSNMACChecker;

public class QueryDevSNMACService {
	private static Logger logger = LoggerFactory.getLogger(QueryDevSNMACService.class);

	public String work(String inXml) {
		QueryDevSNMACChecker checker = new QueryDevSNMACChecker(inXml);
		if (!checker.check())  {
			logger.warn("serviceName[QueryDevSNMACService]cmdId[{}]userinfo[{}]验证未通过，返回：{}",
			new Object[] { checker.getCmdId(), checker.getUserInfo(), checker.getReturnXml() });
			return checker.getReturnXml();
		}
		logger.warn("servicename[QueryDevSNMACService]cmdId[{}]userinfo[{}]参数校验通过，入参为：{}",
		...
		if (null == userInfoMap || userInfoMap.isEmpty() || StringUtil.IsEmpty(userId)) {
			logger.warn("servicename[QueryDevSNMACService]cmdId[{}]userinfo[{}]查无此用户",
		...
		if (StringUtil.IsEmpty(deviceId)) {
			logger.warn("servicename[QueryDevSNMACService]cmdId[{}]userinfo[{}]未绑定设备",
					new Object[] { checker.getCmdId(), checker.getUserInfo()});
			checker.setResult(1002);
		...
		Map<String, String> map = devInfDao.queryDevSNMAC(userId);
		...
		// 记录日志
		new RecordLogDAO().recordDispatchLog(checker, checker.getUserInfo(), "QueryDevSNMAC");
		logger.warn( "servicename[QueryDevSNMACService]cmdId[{}]userinfo[{}]处理结束，返回响应信息:{}",
