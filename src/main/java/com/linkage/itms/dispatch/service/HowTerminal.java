...
import com.linkage.itms.dispatch.obj.HowTerminalChecker;
import com.linkage.itms.obj.ParameValueOBJ;

public class HowTerminal implements IService {

	private static final Logger logger = LoggerFactory
			.getLogger(HowTerminal.class);
	public String work(String inXml) {
		
		HowTerminalChecker checker = new HowTerminalChecker(inXml);
		if (false == checker.check()) {
			logger.error(
					"servicename[HowTerminal]cmdId[{}]userinfo[{}]验证未通过，返回：{}",
					new Object[] { checker.getCmdId(), checker.getUsername(),
							checker.getReturnXml() });
			return checker.getReturnXml();
		}
		logger.warn(
				"servicename[HowTerminal]cmdId[{}]userinfo[{}]参数校验通过，入参为：{}",
				new Object[] { checker.getCmdId(), checker.getUsername(),
						inXml });
		//获取用户信息
		...
		if (null == userInfoMap || userInfoMap.isEmpty()) {
			logger.warn(
					"servicename[HowTerminal]cmdId[{}]userinfo[{}]查无此用户",
					new Object[] { checker.getCmdId(), checker.getUsername()});
		...
		// 用户信息存在   再判断此用户是否绑定了设备
			...
			if ("".equals(deviceId)) {
				logger.warn(
						"servicename[HowTerminal]cmdId[{}]userinfo[{}]此用户没有绑定设备",
						new Object[] { checker.getCmdId(), checker.getUsername()});
			...
			if (-3 == flag) {
				logger.warn(
						"servicename[HowTerminal]cmdId[{}]userinfo[{}]设备正在被操作，无法设置节点值",
						new Object[] { checker.getCmdId(), checker.getUsername()});
			...
			// 设备在线
				...
				logger.warn(
						"servicename[HowTerminal]cmdId[{}]userinfo[{}]设备在线，可以设置节点值",
						new Object[] { checker.getCmdId(), checker.getUsername()});
				ParameValueOBJ pvOBJ = checker.getPvOBJ();
				
				// 调用Corba 设置节点的值
				logger.warn(
						"servicename[HowTerminal]cmdId[{}]userinfo[{}]调用Corba，设置节点值",
						new Object[] { checker.getCmdId(), checker.getUsername()});
				...
				if (0 == retResult || 1 == retResult) {
					...
					// 记录日志
					new RecordLogDAO().recordDispatchLog(checker, checker.getUsername(), "HowTerminal");
					logger.warn(
							"servicename[HowTerminal]cmdId[{}]userinfo[{}]处理结束，返回响应信息:{}",
			...
			// 设备不在线
				...
				logger.warn(
						"servicename[HowTerminal]cmdId[{}]userinfo[{}]设备不在线，无法设置节点值",
						new Object[] { checker.getCmdId(), checker.getUsername()});
				checker.setResult(1000);
				checker.setIsSucc(1);
