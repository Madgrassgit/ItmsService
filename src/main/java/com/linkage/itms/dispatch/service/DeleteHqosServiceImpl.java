 ...
 */
public class DeleteHqosServiceImpl implements IService
{
	private static Logger logger = LoggerFactory.getLogger(DeleteHqosServiceImpl.class);
	HqosUserDeviceDAO userDevDao = new HqosUserDeviceDAO();
	
	@Override
	public String work(String inXml)
	{
		String methodName="DeleteHQoS";
		String returnXml="";
		logger.warn("DeleteHQoS inXml:({})",getStr(inXml));
		DeleteHqosChecker checker = new DeleteHqosChecker(inXml);
		if (!checker.check()) 
		{
			DBOperation.executeUpdate(userDevDao.prossBssSheet(checker,1));
			returnXml= checker.getReturnXml();
			logger.warn("servicename[DeleteHQoS] cmdId[{}],userinfo[{}] 验证未通过，返回：{}",
		...
		if (map==null ||map.isEmpty())
		{
			...
			DBOperation.executeUpdate(userDevDao.prossBssSheet(checker,1));
			returnXml= checker.getReturnXml();
			logger.warn("servicename[DeleteHQoS] cmdId[{}],loid[{}] 无此用户请先开通资料工单，返回：{}",
					checker.getCmdId(),checker.getLoid(),getStr(inXml));
			new RecordLogDAO().recordDispatchLog(checker,1,checker.getLoid(),methodName);
			return returnXml;
		}
		...
		servSqlList.add(userDevDao.stopServiceSql(userId, StringUtil.getIntegerValue(checker.getServTypeId())));
		servSqlList.add(userDevDao.getRemoveHqsServParam(userId, StringUtil.getIntegerValue(checker.getServTypeId())));
		...
		logger.warn("servicename[DeleteHQoS] cmdId[{}],loid[{}] 处理结束，返回响应信息:{}",
