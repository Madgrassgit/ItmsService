...
import com.linkage.itms.dispatch.obj.TestSpeedChecker4SXLT;
 ...
 *
 */
public class TestSpeedService4SXLT implements IService{

	private static Logger logger = LoggerFactory.getLogger(TestSpeedService4SXLT.class);
	private final String methodName = "TestSpeedService4SXLT";
	
	public static void main(String[] args)
	{
		new TestSpeedService4SXLT().work("<root><Param><op_id>1574928979230</op_id><type>0</type><index>43101HW511219078</index><SpeedTest_DiagnosticsState>Requested</SpeedTest_DiagnosticsState><SpeedTest_testMode>serverMode</SpeedTest_testMode><SpeedTest_downloadURL></SpeedTest_downloadURL><SpeedTest_testURL>http://218.27.253.1:8094/speedfile/services/contractrate/contract/rate/</SpeedTest_testURL><SpeedTest_reportURL>http://218.27.253.1:8094/speedfile/services/contractrate/results/insert/</SpeedTest_reportURL></Param></root>");
	}
	@Override
	public String work(String inParam) {
		logger.warn("TestSpeedService4SXLT==>inParam({})",inParam);

		// 解析获得入参
		TestSpeedChecker4SXLT checker = new TestSpeedChecker4SXLT(inParam.trim());
		
		// 验证入参
		if (!checker.check()) {
			logger.warn("入参验证没通过,TestSpeedService4SXLT==>inParam({})",inParam);
	...
	public String doTest(TestSpeedChecker4SXLT checker,String deviceId) throws Exception{
		...
		if(flag==1){
			...
			if("serverMode".equals(checker.getSpeedTest_testMode())){
				if("web".equalsIgnoreCase(checker.getSource())){
					ParameterValueStruct = new ParameterValueStruct[10];
				}
				else{
					ParameterValueStruct = new ParameterValueStruct[6];
				}
			}
			else{
				if("web".equalsIgnoreCase(checker.getSource())){
			...
			if("serverMode".equals(checker.getSpeedTest_testMode())){
				...
				anyObject = new AnyObject();
				/*if(wanPassageWay.charAt(wanPassageWay.length()-1)=='.'){
					wanPassageWay = wanPassageWay.substring(0, wanPassageWay.length()-1);
				}*/
				...
				if("web".equalsIgnoreCase(checker.getSource())){
					...
					ParameterValueStruct[9].setName("InternetGatewayDevice.X_CU_Function.RMS_SpeedTest.downloadURL");
					anyObject = new AnyObject();
					...
					ParameterValueStruct[5] = new ParameterValueStruct();
					ParameterValueStruct[5].setName("InternetGatewayDevice.X_CU_Function.RMS_SpeedTest.downloadURL");
					anyObject = new AnyObject();
					anyObject.para_value = "";
					anyObject.para_type_id = "1";
					ParameterValueStruct[5].setValue(anyObject);
			...
			if(Global.G_instArea.equals(Global.NMGLT))
			{
...
//											fault = XmlToRpc.Fault(soapOBJ.getRpcElement());
					...
					if (PPPoEMap == null){
						...
						if(Global.G_instArea.equals(Global.SXLT))
						{
