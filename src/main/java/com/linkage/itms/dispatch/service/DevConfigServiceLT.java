 ...
 * @since 2017-8-28
 */
public class DevConfigServiceLT implements IService
{
	
	// 日志记录
	private static final Logger logger = LoggerFactory.getLogger(DevConfigServiceLT.class);
	
	private List<HashMap<String, String>> internetlist = new ArrayList<HashMap<String, String>>();
	private HashMap<String, String> internetMap = new HashMap<String, String>();
	...
	private String ststusPatchEPON = null;
	private String txPowerPatchEPON = null;
	private String rxPowerEPON = null;
	...
	public String work(String inXml)
	{
		...
		if (false == checker.check()) {
			logger.error(
					"servicename[DevConfigService4LT]cmdId[{}]userinfo[{}]验证未通过，返回：{}",
		...
		logger.warn(
				"servicename[DevConfigService4LT]cmdId[{}]userinfo[{}]参数校验通过，入参为：{}",
		...
		if (null == userDevInfo || userDevInfo.isEmpty()) {
			logger.warn(
					"servicename[DevConfigService4LT]cmdId[{}]userinfo[{}]查无此用户",
					new Object[] { checker.getCmdId(), checker.getUserInfo() });
			...
			if (StringUtil.IsEmpty(deviceId)) {
				// 未绑定设备
				logger.warn(
						"servicename[DevConfigService4LT]cmdId[{}]userinfo[{}]此客户未绑定",
				...
				try {
					...
					if (1 == flag) {
						...
						if (accessType.indexOf("EPON")!=-1) {
							checkAccessType = ".X_CU_WANEPONInterfaceConfig";
						} else if (accessType.indexOf("GPON")!=-1) {
							checkAccessType = ".X_CU_WANGPONInterfaceConfig";
						} else {
						...
						ststusPatchEPON = "InternetGatewayDevice.WANDevice.1"+checkAccessType+".Status";
						txPowerPatchEPON = "InternetGatewayDevice.WANDevice.1"+checkAccessType+".OpticalTransceiver.TXPower";
						rxPowerEPON = "InternetGatewayDevice.WANDevice.1"+checkAccessType+".OpticalTransceiver.RXPower";
						
						// 采集光功率
						ponInfo(accessType, deviceId, checker);
						// 采集LAN口
						...
						List<String> servListI = corba.getIList(deviceId,
								servListPathI);
						if (null == servListI || servListI.isEmpty()) {
							...
							logger.warn("[{}]获取servListI成功，servListI.size={}",
									deviceId, servListI.size());
						}
						// 获取wan连接
						int countServB = 0;
						...
						for (String i : servListI) {
							...
							if (null == servListJ || servListJ.isEmpty()) {
								servListPathJ = servListPathI + i
										+ ".WANIPConnection.";
								servListJ = corba
										.getIList(deviceId, servListPathI + i
												+ ".WANIPConnection.");
								if (null == servListJ || servListJ.isEmpty()) {
									continue;
								}
							...
							for (String j : servListJ) {
								String servListPathK = servListPathJ + j
										+ ".X_CU_ServiceList";
								ArrayList<ParameValueOBJ> servList = corba
										.getValue(deviceId, servListPathK);
								if (null == servList || servList.size() == 0
										|| null == servList.get(0)
										|| null == servList.get(0).getValue()) {
									continue;
								} else {
									serv = servList.get(0).getValue();//
									logger.warn("serv="+serv);
									if ("IPTV".equalsIgnoreCase(serv)) {
										countServB++;
										servFlagIptv = true;
										logger.warn("[{}]获取IPTV成功", deviceId);

										// 采集 X_CT-X_CU_MulticastVlan
										String mulPath = servListPathJ + j
												+ ".X_CU_MulticastVlan";
										String bindPortPath = servListPathJ + j
												+ ".X_CU_LanInterface";
										String connTypePath = servListPathJ + j
												+ ".ConnectionType";
										String vlanPath = servListPathI + i + ".X_CU_VLAN";

										String[] gatherPath = new String[] {
												mulPath, bindPortPath,
												connTypePath, vlanPath };
										...
										ArrayList<ParameValueOBJ> wifiStateLlist = corba
												.getValue(deviceId,
														wifiStatePath);
										
										if (null == wifiStateLlist || wifiStateLlist.isEmpty()) {
											logger.warn(
													"wifiStatus节点值没有获取到，请确认节点路径是否正确，deviceId:({})",
													deviceId);
											internetMap.put("status", "");
										} else {
											internetMap.put("status",
													wifiStateLlist.get(0)
															.getValue());
										}
										...
										logger.warn("bindPortPath="+bindPortPath);
										logger.warn("connTypePath="+connTypePath);
										logger.warn("vlanPath="+vlanPath);
										String[] gatherPath = new String[] {
												bindPortPath, connTypePath,
												vlanPath };
						...
						} else if (accessType.indexOf("EPON")!=-1) {
		...
		logger.warn(
				"servicename[DevConfigService4LT]cmdId[{}]userinfo[{}]处理结束，返回响应信息:{}",
				new Object[] { checker.getCmdId(), checker.getUserInfo(),
						returnXml });
	...
	private void ponInfo(String accessType, String deviceId,
			DevConfigCheckerNew checker) {
		...
		logger.warn("this.ststusPatchEPON="+this.ststusPatchEPON);
		logger.warn("this.txPowerPatchEPON="+this.txPowerPatchEPON);
		logger.warn("this.rxPowerEPON="+this.rxPowerEPON);
		String[] gatherPath = new String[] { this.ststusPatchEPON,
				this.txPowerPatchEPON, this.rxPowerEPON, };
