...
import com.linkage.itms.dispatch.obj.DevConfigChecker;
 ...
 * @version 1.0
 * @since 2011-5-16 下午03:42:40
 * @category com.linkage.itms.dispatch.service
 * @copyright 南京联创科技 网管科技部
 * 
 * 注释 by zhangchy 2012-03-07
 * 
 * 根据需求单JSDX_ITMS-REQ-20120220-LUHJ-006以及ItmsService 1.5.8 版本 2.11 设备配置查询接口的要求，
 * 回参不需要显示"配置信息检查结果(result)"，顾将wideNetMsg，iptvMsg，voipMsg以及Internet，IPTV，VOIP的配置检查均注释
 * 
 */
public class DevConfigService implements IService
{

	// 日志记录
	private static final Logger logger = LoggerFactory.getLogger(DevConfigService.class);
...
//	private List<HashMap<String, String>> voipdevicelist;
	...
	public String work(String inXml)
	{
		...
		DevConfigChecker devConfiger = new DevConfigChecker(inXml);
		...
		if (null == userDevInfo || userDevInfo.isEmpty()) {
			logger.warn(
					"servicename[DevConfigService]cmdId[{}]userinfo[{}]查无此用户",
					new Object[] { devConfiger.getCmdId(), devConfiger.getUserInfo()});
			...
			if (StringUtil.IsEmpty(deviceId)) {
				...
				logger.warn(
						"servicename[DevConfigService]cmdId[{}]userinfo[{}]getCpeParams设备配置信息采集结果：{}",
...
//					resultMap.put("inter_result", wideNetMsg); 
					configInfo.put("inter_list", internetlist);
					
					//IPTV
//					根据需求单JSDX_ITMS-REQ-20120220-LUHJ-006及ItmsService接口文档，返回的参数不需要"配置信息检查结果"(result)节点
//					resultMap.put("iptv_result", iptvMsg);
					configInfo.put("iptv_list", iptvlist);
					
					//VOIP
//					根据需求单JSDX_ITMS-REQ-20120220-LUHJ-006及ItmsService接口文档，返回的参数不需要"配置信息检查结果"(result)节点
//					resultMap.put("voip_result", voipMsg);
...
// 		回参不需要返回"配置信息检查结果" 注释 by zhangchy 2012-03-07    -----
//		Map<String, List<Map<String, String>>> preconfmap = dao.getPreConfInfo(parecityid, shortName, accessType);
...
// 		根据需求单JSDX_ITMS-REQ-20120220-LUHJ-006及ItmsService接口文档，返回的参数不需要"配置信息检查结果"(result)节点
// 		注释 by zhangchy 2012-03-07   begin -----
//		String userId = dao.getUserByDevice(deviceId);
//		if ("DSL".equals(accessType))
//		{
//			// 宽带上网预配置
//			logger.warn("DSL宽带上网预配置");
//			internetDSLConf(preconfmap);
//			// IPTV预配置
//			logger.warn("DSLIPTV预配置");
//			iptvtDSLConf(preconfmap);
//			// VOIP预配置
//			logger.warn("DSLVOIP预配置");
//			voipDSLConf(preconfmap,userId);
//			
//		}
//		else
//		{
//			logger.error("zhangshimin1");
//			// PON上网业务预配置
//			internetPONConf(preconfmap, userId);
//			logger.error("zhangshimin2");
//			// IPTV预配置
//			iptvPONConf(preconfmap, userId);
//		}
//		logger.error("zhangshimin3");
//		// VOIP预配置
//		voipPONConfig(preconfmap, userId);
//		
// 		注释 by zhangchy 2012-03-07   end -----
	}
...
//				{
//					boolean falg = false;
//					Map<String, String> tmap = (Map<String, String>) voipuserlist
//							.get(i);
//					for (int j = 0; j < voipdevicelist.size(); j++)
//					{
//						Map<String, String> rmap = (Map<String, String>) voipdevicelist
//								.get(i);
//						if (StringUtil.getStringValue(tmap.get("voip_username"))
//								.equals(StringUtil.getStringValue(rmap
//										.get("username")))
//								&& StringUtil.getStringValue(tmap.get("line_id"))
//										.equals(StringUtil.getStringValue(rmap
//												.get("line_id")))
//								&& StringUtil.getStringValue(tmap.get("prox_serv"))
//										.equals(StringUtil.getStringValue(rmap
//												.get("prox_serv")))
//								&& StringUtil.getStringValue(tmap.get("prox_port"))
//										.equals(StringUtil.getStringValue(rmap
//												.get("prox_port"))))
//						{
//							falg = true;
...
//	public List<HashMap<String, String>> getVoipdevicelist()
//	{
//		return voipdevicelist;
//	}
//
//	public void setVoipdevicelist(List<HashMap<String, String>> voipdevicelist)
//	{
...
//	}
	...
	public void setIGMPSnoopingList(List<HashMap<String, String>> snoopingList) {
		IGMPSnoopingList = snoopingList;
	}
