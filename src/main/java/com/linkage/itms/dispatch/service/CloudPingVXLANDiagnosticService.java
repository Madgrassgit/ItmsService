...
import com.linkage.itms.dispatch.obj.CloudPingVXLANDiagnosticChecker;
...
public class CloudPingVXLANDiagnosticService implements IService {

	// 日志
	private static final Logger logger = LoggerFactory.getLogger(CloudPingVXLANDiagnosticService.class);

	@Override
	public String work(String inXml) {
		CloudPingVXLANDiagnosticChecker checker = new CloudPingVXLANDiagnosticChecker(inXml);
		try {
			// 验证入参格式是否正确
			if (!checker.check()) {
				logger.warn("servicename[CloudPingVXLANDiagnosticService]cmdId[{}]userinfo[{}]验证未通过，返回：{}",
						new Object[] {checker.getCmdId(), checker.getUserInfo(), inXml});
				return checker.getReturnXml();
			}
			logger.warn("servicename[CloudPingVXLANDiagnosticService]cmdId[{}]userinfo[{}]参数校验通过，入参为：{}",
					new Object[] {checker.getCmdId(), checker.getUserInfo(), inXml});
...
//			}
			...
			logger.warn("CloudPingVXLANDiagnosticService is error:", e);
		}
	 ...
	 * @return
	 */
	private String getReturnXml(CloudPingVXLANDiagnosticChecker checker) {
		new RecordLogDAO().recordDispatchLog(checker, "CloudPingVXLANDiagnostic", checker.getCmdId());
		logger.warn("servicename[CloudPingVXLANDiagnosticService]cmdId[{}]处理结束，返回响应信息:{}",
	 ...
	 * @return
	 */
	public String PingList(String gw_type, String device_id, CloudPingVXLANDiagnosticChecker checker) {
...
//		ParameterValueStruct[] ParameterValueStruct = new ParameterValueStruct[7];
		...
		ParameterValueStruct[1].setName("InternetGatewayDevice.IPPingDiagnostics.SourceIpAddress");
		...
		ParameterValueStruct[7].setName("InternetGatewayDevice.IPPingDiagnostics.Interface");
		anyObject = new AnyObject();
		anyObject.para_value = "";
		if ("sd_dx".equals(Global.G_instArea)) {
			anyObject.para_value = "InternetGatewayDevice.X_CT-COM_VXLAN.VXLANConfig.1.";
		}
		anyObject.para_type_id = "1";
