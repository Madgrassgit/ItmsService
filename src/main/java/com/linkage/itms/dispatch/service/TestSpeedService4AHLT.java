...
import com.linkage.itms.dispatch.obj.TestSpeedChecker4AHLT;
import com.linkage.itms.dispatch.obj.TestSpeedChecker4JLLT;
 ...
 *
 */
public class TestSpeedService4AHLT implements IService{

	private static Logger logger = LoggerFactory.getLogger(TestSpeedService4AHLT.class);
	private final String methodName = "TestSpeedService4AHLT";
	
	public static void main(String[] args)
	{
		new TestSpeedService4AHLT().work("<root><Param><op_id>1574928979230</op_id><type>0</type><index>43101HW511219078</index><SpeedTest_DiagnosticsState>Requested</SpeedTest_DiagnosticsState><SpeedTest_testMode>serverMode</SpeedTest_testMode><SpeedTest_downloadURL></SpeedTest_downloadURL><SpeedTest_testURL>http://218.27.253.1:8094/speedfile/services/contractrate/contract/rate/</SpeedTest_testURL><SpeedTest_reportURL>http://218.27.253.1:8094/speedfile/services/contractrate/results/insert/</SpeedTest_reportURL></Param></root>");
	}
	@Override
	public String work(String inParam) {
		logger.warn("TestSpeedService4AHLT==>inParam({})",inParam);

		// 解析获得入参
		TestSpeedChecker4AHLT checker = new TestSpeedChecker4AHLT(inParam.trim());
		
		// 验证入参
		if (!checker.check()) {
			logger.warn("入参验证没通过,TestSpeedService4AHLT==>inParam({})",inParam);
	...
	public String doTest(TestSpeedChecker4AHLT checker,String deviceId) throws Exception{
		ACSCorba acsCorba = new ACSCorba();
		GetDeviceOnLineStatus getStatus = new GetDeviceOnLineStatus();
		int flag = getStatus.testDeviceOnLineStatus(deviceId, acsCorba);
		String wanPassageWay = "";
		if(flag==1){
			wanPassageWay = checker.getWanPassageWay();
			...
			if(StringUtil.IsEmpty(wanPassageWay)){
				...
				if(null != wanPassageWayMap && !wanPassageWayMap.isEmpty()){
					...
					checker.setResultDesc("获取wan通道失败");
			...
			ParameterValueStruct[] ParameterValueStruct = new ParameterValueStruct[10];
			
			
			
			ParameterValueStruct[0] = new ParameterValueStruct();
			ParameterValueStruct[0].setName("InternetGatewayDevice.X_CU_Function.RMS_SpeedTest.DiagnosticsState");
			AnyObject anyObject = new AnyObject();
			anyObject.para_value = checker.getSpeedTest_DiagnosticsState();
			anyObject.para_type_id = "1";
			ParameterValueStruct[0].setValue(anyObject);
			
			
			ParameterValueStruct[1] = new ParameterValueStruct();
			ParameterValueStruct[1].setName("InternetGatewayDevice.X_CU_Function.RMS_SpeedTest.testMode");
			...
			ParameterValueStruct[2].setName("InternetGatewayDevice.X_CU_Function.RMS_SpeedTest.downloadURL");
			anyObject = new AnyObject();
			...
			ParameterValueStruct[3].setName("InternetGatewayDevice.X_CU_Function.RMS_SpeedTest.testURL");
			anyObject = new AnyObject();
			anyObject.para_value = checker.getSpeedTest_testURL();
			anyObject.para_type_id = "1";
			...
			ParameterValueStruct[5].setName("InternetGatewayDevice.X_CU_Function.RMS_SpeedTest.backgroundsize");
			anyObject = new AnyObject();
			anyObject.para_value = checker.getBackgroundsize();
			anyObject.para_type_id = "1";
			ParameterValueStruct[5].setValue(anyObject);
			
			ParameterValueStruct[6] = new ParameterValueStruct();
			ParameterValueStruct[6].setName("InternetGatewayDevice.X_CU_Function.RMS_SpeedTest.pppoeName");
			anyObject = new AnyObject();
			...
			ParameterValueStruct[7].setName("InternetGatewayDevice.X_CU_Function.RMS_SpeedTest.Eupppoename");
			anyObject = new AnyObject();
			...
			ParameterValueStruct[8].setName("InternetGatewayDevice.X_CU_Function.RMS_SpeedTest.Eupassword");
			anyObject = new AnyObject();
			anyObject.para_value = checker.getPassword();
			anyObject.para_type_id = "1";
			ParameterValueStruct[8].setValue(anyObject);
			
			ParameterValueStruct[9] = new ParameterValueStruct();
			ParameterValueStruct[9].setName("InternetGatewayDevice.X_CU_Function.RMS_SpeedTest.WANInterface");
			anyObject = new AnyObject();
			anyObject.para_value = wanPassageWay;
			anyObject.para_type_id = "1";
			ParameterValueStruct[9].setValue(anyObject);
			...
			String testMode = "InternetGatewayDevice.X_CU_Function.RMS_SpeedTest.testMode";
...
//											fault = XmlToRpc.Fault(soapOBJ.getRpcElement());
					...
					if (PPPoEMap == null){
						...
						checker.setTestMode(""+PPPoEMap.get(testMode));
	...
	/*if("serverMode".equals(checker.getSpeedTest_testMode())){
		if("web".equalsIgnoreCase(checker.getSource())){
		...
		if("web".equalsIgnoreCase(checker.getSource())){
			...
			ParameterValueStruct = new ParameterValueStruct[4];
		}
	}
