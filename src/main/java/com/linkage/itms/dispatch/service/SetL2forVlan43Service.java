 ...
 *
 */
public class SetL2forVlan43Service implements IService{

	private static Logger logger = LoggerFactory.getLogger(SetL2forVlan43Service.class);

	public String work(String inXml)
	{
		VlanStatusServiceDao dao=new VlanStatusServiceDao();
		logger.warn("VlanStatus==>inXml({})",inXml);
		VlanStatusChecker checker = new VlanStatusChecker(inXml,true);
		...
		List<HashMap<String, String>> devList=userDevDao.queryDeviceInfoByLoidWithModel(checker.getLoid());
		...
		String loidSupport=dao.selectLoidInTempTable(devList.get(0).get("device_model"));
		...
		if (-6 == flag) {
			...
			for (int i = 0; i < wanConnPathsList.size(); i++)
			{
				...
				if (namepath.indexOf(vlanIdPath) >= 0 | namepath.indexOf(wanServiceList) >= 0
						|namepath.indexOf(wanLanInterface) >= 0 )
				{
			...
			if (serviceListList.size() == 0 || serviceListList.isEmpty())
			{
					...
					if (paramName.indexOf(vlanIdPath) >= 0)
					{
						if (!StringUtil.IsEmpty(entry.getValue())){//X_CT-COM_ServiceList的值为INTERNET的时候，此节点路径即为要删除的路径
							if(StringUtil.getIntegerValue(entry.getValue()) == 43){
								setPath = wanConnPath + j + wanPPPConnection + "1" + wanLanInterface;
							}
							if(StringUtil.getIntegerValue(entry.getValue()) == 41){
								setL1 = wanConnPath + j + wanPPPConnection + "1" + wanLanInterface;
							}
						}
					}
				...
				if(setPath != null && !"".equals(setPath)){
					...
					pvObj.setValue(L1);
					pvObjList.add(pvObj);
					logger.warn("[{}]开始下vlan发参数[{}],[{}]",deviceId,setPath,setL1);
					int result = acsCorba.setValue(deviceId, pvObjList);
					if(result != 1 && result != 0){
						logger.warn("[{}]下发参数失败[{}]", deviceId, result);
						checker.setResult(1000);
						checker.setResultDesc("未知错误【下发参数失败】");
						return  checker.getReturnXml();
					}
					logger.warn("[{}]下发参数成功[{}]", deviceId, result);
					return  checker.getReturnXml();
				}
			}
		}else {
