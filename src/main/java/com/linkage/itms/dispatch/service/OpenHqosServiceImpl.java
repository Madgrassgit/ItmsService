 ...
 */
public class OpenHqosServiceImpl implements IService
{
	private static Logger logger = LoggerFactory.getLogger(OpenHqosServiceImpl.class);
	HqosUserDeviceDAO userDevDao = new HqosUserDeviceDAO();
	
	@Override
	public String work(String inXml)
	{
		String methodName="OpenHQoS";
		String returnXml="";
		logger.warn("OpenHQoS inXml:({})",getStr(inXml));
		OpenHqosChecker checker = new OpenHqosChecker(inXml);
		if (!checker.check()) 
		{
			DBOperation.executeUpdate(userDevDao.prossBssSheet(checker,1));
			returnXml= checker.getReturnXml();
			logger.warn("servicename[OpenHQoS] cmdId[{}],userinfo[{}] 验证未通过，返回：{}",
		...
		if (map==null ||map.isEmpty())
		{
			...
			DBOperation.executeUpdate(userDevDao.prossBssSheet(checker,1));
			returnXml= checker.getReturnXml();
			logger.warn("servicename[OpenHQoS] cmdId[{}],loid[{}] 无此用户请先开通资料工单，返回：{}",
					checker.getCmdId(),checker.getLoid(),getStr(inXml));
			new RecordLogDAO().recordDispatchLog(checker,1,checker.getLoid(),methodName);
			return returnXml;
		}
		...
		int servTypeId = StringUtil.getIntegerValue(checker.getServTypeId());
		hasServUser = userDevDao.servUserIsExistsByServName(userId, servTypeId);
		if (!hasServUser)
		{
			// 业务用户不存在
			servSqlList.add(userDevDao.saveServUserSqlNew(checker, userId));
			servSqlList.add(userDevDao.getSaveHqsServParam(checker, userId));
		}
		else
		{
		...
		logger.warn("servicename[OpenHQoS] cmdId[{}],loid[{}] 处理结束，返回响应信息:{}",
