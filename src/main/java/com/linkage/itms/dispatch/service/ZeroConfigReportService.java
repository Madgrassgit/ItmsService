 ...
 */
public class ZeroConfigReportService implements IService
{

	private static final Logger logger = LoggerFactory
			.getLogger(ZeroConfigReportService.class);
	/**
	 * 业务类型：家庭网关e8-b
	 */
	...
	{
		logger.warn("ZeroConfigReportService,inParam:({})", inXml);
		ZeroConfigReportChecker checker = new ZeroConfigReportChecker(inXml);
		// 验证入参格式是否正确
		if (false == checker.check())
		{
			logger.error(
					"servicename[ZeroConfigReportService],cmdId[{}],验证未通过,返回：{}",
					new Object[] { checker.getCmdId(), checker.getReturnXml() });
			logger.warn(
					"ZeroConfigReportService,cmdId[{}]验证未通过,返回：{}",
					new Object[] { checker.getCmdId(), checker.getReturnXml() });
		...
		{
			logger.warn("servicename[ZeroConfigReportService]cmdId[{}]userinfo[{}]成功,暂未查出此用户",
		...
		else
		{
			...
			Map<String, String> devMap = devDao.queryDevInfo(checker.getServiceType(),
					checker.getDevSn());
			if (null == devMap || devMap.isEmpty()
					|| StringUtil.IsEmpty(devMap.get("device_id")))
			{
				logger.warn(
						"servicename[ZeroConfigReportService]cmdId[{}]deviceSn[{}]成功,暂未查出此设备",
		...
		logger.warn("ZeroConfigReportService, returnXML:{}",returnXml);
		return returnXml;
	}
