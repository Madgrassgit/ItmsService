...
import com.linkage.itms.dispatch.obj.CloudQueryVXLANGetWayChecker;


public class CloudQueryVXLANGetWayService implements IService {

	// 日志
	private static final Logger logger = LoggerFactory.getLogger(CloudQueryVXLANGetWayService.class);

	@Override
	public String work(String inXml) {
		CloudQueryVXLANGetWayChecker checker = new CloudQueryVXLANGetWayChecker(inXml);
		try {
			// 验证入参格式是否正确
			if (!checker.check()) {
				logger.warn("servicename[CloudQueryVXLANGetWayService]cmdId[{}]userinfo[{}]验证未通过，返回：{}",
						new Object[] {checker.getCmdId(), checker.getUserInfo(), inXml});
				return checker.getReturnXml();
			}
			logger.warn("servicename[CloudQueryVXLANGetWayService]cmdId[{}]userinfo[{}]参数校验通过，入参为：{}",
					new Object[] {checker.getCmdId(), checker.getUserInfo(), inXml});
...
//			checker.setLoidPrev("");
			...
			// IsVxlan
			checker.setIsVxlan(ispDao.queryVxlanEnable(userId));
			// WanType
			...
			checker.setStatus("0");
			// VxlanStatus
			checker.setVxlanStatus("0");
			checker.setResult(0);
			checker.setResultDesc("成功");
		}
		catch (Exception e) {
			logger.warn("CloudQueryVXLANGetWayService is error:", e);
		}
	 ...
	 * @return
	 */
	private String getReturnXml(CloudQueryVXLANGetWayChecker checker) {
		new RecordLogDAO().recordDispatchLog(checker, "CloudQueryVXLANGetWayService", checker.getCmdId());
		logger.warn("servicename[CloudQueryVXLANGetWayService]cmdId[{}]处理结束，返回响应信息:{}",
