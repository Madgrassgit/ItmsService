...
import com.linkage.itms.dispatch.obj.ServiceDoneChecker;
 ...
 * @date 2010-6-21
 */
public class ServiceDoneService implements IService
{

	private static Logger logger = LoggerFactory.getLogger(ServiceDoneService.class);
	...
	public String work(String inXml)
	{
		...
		if (1 == serviceDoner.getSearchType())
		{
			...
			{
				logger.warn(
						"servicename[ServiceDoneService]cmdId[{}]userinfo[{}]查无此用户",
						new Object[] { serviceDoner.getCmdId(), serviceDoner.getUserInfo()});
			...
			else
			{
				...
				{
					logger.warn(
							"servicename[ServiceDoneService]cmdId[{}]userinfo[{}]用户未绑定设备",
							new Object[] { serviceDoner.getCmdId(), serviceDoner.getUserInfo()});
				...
				else
				{
					...
					{// 属地不匹配
						logger.warn(
								"servicename[ServiceDoneService]cmdId[{}]userinfo[{}]属地不匹配，查无此用户",
								new Object[] { serviceDoner.getCmdId(), serviceDoner.getUserInfo()});
						serviceDoner.setResult(1003);
						serviceDoner.setResultDesc("查无此用户");
					}
					else
					{// 属地匹配
		...
		else if (2 == serviceDoner.getSearchType())
		{
			...
			{
				logger.warn(
						"servicename[ServiceDoneService]cmdId[{}]userinfo[{}]查无此设备",
						new Object[] { serviceDoner.getCmdId(), serviceDoner.getUserInfo()});
			...
			{
				logger.warn(
						"servicename[ServiceDoneService]cmdId[{}]userinfo[{}]查询到多台设备",
						new Object[] { serviceDoner.getCmdId(), serviceDoner.getUserInfo()});
			...
			else
			{
				...
				{
					logger.warn(
							"servicename[ServiceDoneService]cmdId[{}]userinfo[{}].ITMS未绑devSn({})",
				...
				else
				{
					...
					{// 属地不匹配
						logger.warn(
								"servicename[ServiceDoneService]cmdId[{}]userinfo[{}]属地不匹配，查无此设备",
								new Object[] { serviceDoner.getCmdId(), serviceDoner.getUserInfo()});
						serviceDoner.setResult(1005);
						serviceDoner.setResultDesc("查无此设备");
					}
		...
		if (true == isBinded)
		{
			...
			{
				logger.warn(
						"servicename[ServiceDoneService]cmdId[{}]userinfo[{}]设备繁忙或者业务正在下发，请稍候重试",
			...
			else
			{
				...
				{
					logger.warn(
							"servicename[ServiceDoneService]cmdId[{}]userinfo[{}]用户为受理任何业务",
							new Object[] { serviceDoner.getCmdId(),
									serviceDoner.getUserInfo() });
					serviceDoner.setResult(1009);
				...
				{ // 全业务下发
					if (0 == serviceDoner.getServiceType())
					{
						...
						{
							logger.warn(
									"servicename[ServiceDoneService]cmdId[{}]userinfo[{}]设备[{}]全业务下发，调用配置模块失败",
									new Object[] { serviceDoner.getCmdId(),
											serviceDoner.getUserInfo(), deviceId });
							serviceDoner.setResult(1000);
							serviceDoner.setResultDesc("未知错误，请稍后重试");
						}
					}
					else
					{ // 特定业务下发
						...
						for (HashMap<String, String> servUserMap : servUserMapList)
						{
									...
									.getIntegerValue(servUserMap.get("serv_type_id")))
							{
								...
								{
									logger.warn(
											"servicename[ServiceDoneService]cmdId[{}]userinfo[{}]设备[{}]下发特定业务，调用后台预读模块失败，业务类型为：[{}]",
											new Object[] { serviceDoner.getCmdId(),
													serviceDoner.getUserInfo(), deviceId,
													serviceDoner.getServiceType() });
									serviceDoner.setResult(1000);
						...
						{
							logger.warn(
									"servicename[ServiceDoneService]cmdId[{}]userinfo[{}]用户未受理该业务，无法进行业务下发",
									new Object[] { serviceDoner.getCmdId(),
											serviceDoner.getUserInfo() });
							serviceDoner.setResult(1009);
		...
		new RecordLogDAO().recordDispatchLog(serviceDoner, serviceDoner.getUserInfo(),
				"ServiceDoneService");
		logger.warn(
				"servicename[ServiceDoneService]cmdId[{}]userinfo[{}]处理结束，返回响应信息:{}",
