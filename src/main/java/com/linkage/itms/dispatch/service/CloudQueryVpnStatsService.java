...
import com.linkage.itms.dispatch.obj.CloudQueryVpnStatsChecker;
import com.linkage.itms.obj.ParameValueOBJ;


public class CloudQueryVpnStatsService implements IService {

	// 日志
	private static final Logger logger = LoggerFactory.getLogger(CloudQueryVpnStatsService.class);

	@Override
	public String work(String inXml) {
		CloudQueryVpnStatsChecker checker = new CloudQueryVpnStatsChecker(inXml);
		try {
			// 验证入参格式是否正确
			if (!checker.check()) {
				logger.warn("servicename[CloudQueryVpnStatsService]cmdId[{}]userinfo[{}]验证未通过，返回：{}",
						new Object[] {checker.getCmdId(), checker.getUserInfo(), inXml});
				return checker.getReturnXml();
			}
			logger.warn("servicename[CloudQueryVpnStatsService]cmdId[{}]userinfo[{}]参数校验通过，入参为：{}",
					new Object[] {checker.getCmdId(), checker.getUserInfo(), inXml});
...
//			checker.setLoidPrev("");
			...
			// 设备在线
				...
				if (null == jList || jList.size() == 0 || jList.isEmpty()) {
					logger.warn("servicename[CloudQueryVpnStatsService][{}]获取" + ipsecPath + "下实例号失败", deviceId);
					checker.setResult(1003);
				...
				for (String j : jList) {
					String[] gatherPath = new String[] {
							"InternetGatewayDevice.VPN.X_CT-COM_IPSecVPN." + j + ".ConnectionStatus"
					};
					ArrayList<ParameValueOBJ> objList = corba.getValue(deviceId, gatherPath);
					if (null != objList && !objList.isEmpty()) {
						checker.setIpescVpnStatus(objList.get(0).getValue());
					}
					else {
			...
			logger.warn("CloudQueryVpnStatsService is error:", e);
		}
		logger.warn("return=({})", checker.getReturnXml());
	 ...
	 * @return
	 */
	private String getReturnXml(CloudQueryVpnStatsChecker checker) {
		new RecordLogDAO().recordDispatchLog(checker, "CloudQueryVpnStatsService", checker.getCmdId());
		logger.warn("servicename[CloudQueryVpnStatsService]cmdId[{}]处理结束，返回响应信息:{}",
