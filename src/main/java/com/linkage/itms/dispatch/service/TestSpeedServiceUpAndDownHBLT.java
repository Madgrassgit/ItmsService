...
import com.linkage.itms.dispatch.obj.TestSpeedCheckerUpAndDownHBLT;
 ...
 *
 */
public class TestSpeedServiceUpAndDownHBLT implements IService{

	private static Logger logger = LoggerFactory.getLogger(TestSpeedServiceUpAndDownHBLT.class);
	@Override
	public String work(String inParam) {
		logger.warn("TestSpeedServiceUpAndDownHBLT==>inParam({})",inParam);

		// 解析获得入参
		TestSpeedCheckerUpAndDownHBLT checker = new TestSpeedCheckerUpAndDownHBLT(inParam.trim());
		
		// 验证入参
		if (!checker.check()) {
			logger.warn("入参验证没通过,TestSpeedServiceUpAndDownHBLT==>inParam({})",inParam);
		...
		if(!"IP_Routed".equals(checker.getConnType())){
			...
			if(null != list && list.size()>0){
				...
				logger.warn("TestSpeedServiceUpAndDownHBLT==>ReturnXml:"+checker.getReturnXml());
				return checker.getReturnXml();
			}
		}
		...
		logger.warn(
				"servicename[TestSpeedServiceUpAndDownHBLT]cmdId[{}]userinfo[{}]处理结束，返回响应信息:{}",
	...
	public String doTest(TestSpeedCheckerUpAndDownHBLT checker,String deviceId) throws Exception{
		...
		if(flag==1){
			...
			//桥接
			if(!"IP_Routed".equals(checker.getConnType())){
				if (checker.getType().equals("UpLink")) {
					//上行
					ParameterValueStruct = new ParameterValueStruct[9];
				}else {
					//下行 目前为了兼容现网设备 type节点先不下发
			...
			if(!"IP_Routed".equals(checker.getConnType())){
				...
				ParameterValueStruct[8].setName("InternetGatewayDevice.X_CU_Function.RMS_SpeedTest.Type");
				anyObject = new AnyObject();
				anyObject.para_value =checker.getType();
				anyObject.para_type_id = "1";
				...
				if (checker.getType().equals("UpLink")) {
				ParameterValueStruct[5] = new ParameterValueStruct();
				ParameterValueStruct[5].setName("InternetGatewayDevice.X_CU_Function.RMS_SpeedTest.Type");
				anyObject = new AnyObject();
				anyObject.para_value =checker.getType();
				anyObject.para_type_id = "1";
				ParameterValueStruct[5].setValue(anyObject);
