...
import com.linkage.itms.dispatch.obj.ModifyChannelChecker;
 ...
 */
public class ModifyChannelService implements IService
{

	private static final Logger logger = LoggerFactory
			.getLogger(ModifyChannelService.class);

	@Override
	public String work(String inXml)
	{
		logger.warn("ModifyChannelService, inParam:({})", inXml);
		ModifyChannelChecker checker = new ModifyChannelChecker(inXml);
		// 验证入参格式是否正确
		if (false == checker.check())
		{
			logger.error("servicename[ModifyChannelService],cmdId[{}],验证未通过，返回：{}",
					new Object[] { checker.getCmdId(), checker.getReturnXml() });
			logger.warn("ModifyChannelService,cmdId[{}],return=({})", checker.getCmdId(),
		...
		// 获取入参的cityid 对应的子属地及自己
		ArrayList<String> cityIDs = CityDAO.getAllNextCityIdsByCityPid(checker
				.getCityId());
		if (checker.getUserInfoType() == 1)
		{
			userMap = qdDao.queryUserByNetAccount(checker.getUserInfo(), cityIDs);
		}
		else if (checker.getUserInfoType() == 2)
		{
			userMap = qdDao.queryUserByLoid(checker.getUserInfo(), cityIDs);
		}
		else if (checker.getUserInfoType() == 3)
		{
			userMap = qdDao.queryUserByIptvAccount(checker.getUserInfo(), cityIDs);
		}
		else if (checker.getUserInfoType() == 4)
		{
			userMap = qdDao.queryUserByVoipPhone(checker.getUserInfo(), cityIDs);
		}
		else if (checker.getUserInfoType() == 5)
		{
			userMap = qdDao.queryUserByVoipAccount(checker.getUserInfo(), cityIDs);
		}
...
//		}
		...
		else if (1 == flag)
		{
			...
			String channelValue = checker.getOperateType();
			...
			else
			{
				...
				if (0 == retResult || 1 == retResult)
				{
					...
					new RecordLogDAO().recordDispatchLog(checker, checker.getUserInfo(),
							"ModifyChannelService");
					logger.warn(
							"servicename[ModifyChannelService]cmdId[{}]userinfo[{}]处理结束，返回响应信息:{}",
