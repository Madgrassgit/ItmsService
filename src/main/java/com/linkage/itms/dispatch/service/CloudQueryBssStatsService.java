...
import com.linkage.itms.dispatch.obj.CloudQueryBssStatsChecker;


public class CloudQueryBssStatsService implements IService {

	// 日志
	private static final Logger logger = LoggerFactory.getLogger(CloudQueryBssStatsService.class);

	@Override
	public String work(String inXml) {
		CloudQueryBssStatsChecker checker = new CloudQueryBssStatsChecker(inXml);
		try {
			// 验证入参格式是否正确
			if (!checker.check()) {
				logger.warn("servicename[CloudQueryBssStatsService]cmdId[{}]userinfo[{}]验证未通过，返回：{}",
						new Object[] {checker.getCmdId(), checker.getUserInfo(), inXml});
				return checker.getReturnXml();
			}
			logger.warn("servicename[CloudQueryBssStatsService]cmdId[{}]userinfo[{}]参数校验通过，入参为：{}",
					new Object[] {checker.getCmdId(), checker.getUserInfo(), inXml});
...
//			checker.setLoidPrev("");
			...
			if (ispDao.queryIpsecServCount(StringUtil.getLongValue(userId)) == 0) {
				logger.warn(
						"servicename[CloudQueryBssStatsService]cmdId[{}]userinfo[{}]没有对应Ipsec业务记录",
						new Object[] { checker.getCmdId(), checker.getUserInfo() });
				checker.setResult(1009);
				checker.setResultDesc("没有对应Ipsec业务记录");
				return getReturnXml(checker);
			}
			checker.setBssStats(ispDao.queryServStatus(StringUtil.getLongValue(userId)));
			checker.setResult(0);
			checker.setResultDesc("成功");
		}
		catch (Exception e) {
			logger.warn("CloudQueryBssStatsService is error:", e);
		}
		return getReturnXml(checker);
	}
	 ...
	 * @return
	 */
	private String getReturnXml(CloudQueryBssStatsChecker checker) {
		new RecordLogDAO().recordDispatchLog(checker, "CloudQueryBssStatsService", checker.getCmdId());
		logger.warn("servicename[CloudQueryBssStatsService]cmdId[{}]处理结束，返回响应信息:{}",
