...
public class GetCpeParameterByNodeNameXML  extends BaseDealXML {
	public GetCpeParameterByNodeNameXML(String methodName) {
		super(methodName);
	}


	private static Logger logger = LoggerFactory.getLogger(GetCpeParameterByNodeNameXML.class);
	...
	public Document getXML(String inXml,String[] parameterNames) {
		try
		{
			...
			{
				this.result ="-3";
				this.errMsg ="参数值parameterNames为空";
				logger.warn(methodName+"["+opId+"]参数值parameterNames为空");
				return null;
			}
			return inDocument;
		}catch(Exception e){
			logger.error(methodName+"["+opId+"] Excetion occured!-e:{}", ExceptionUtils.getStackTrace(e));
			return null;
		}
	}
