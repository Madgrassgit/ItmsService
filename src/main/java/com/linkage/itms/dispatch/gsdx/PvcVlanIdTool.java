package com.linkage.itms.dispatch.gsdx;
...
import com.linkage.itms.dispatch.gsdx.obj.WanConnObj;
import com.linkage.itms.dispatch.gsdx.obj.WanConnSessObj;
...
public class PvcVlanIdTool {
	...
	public String getServiceListAndVlanId(){
		...
		{// 将获取的参数值进行过滤和组装，放入集合中，其数据结构为wanConnMap<j,WanConnObj.WanConnSessMap<k#ipType,WanConnSessMap>>
			...
			if(paramName.indexOf(".", wanConnPath.length()) == -1){
				continue;
			}
			...
			if (paramName.indexOf(wanServiceList) >= 0)
			{
				...
				/*if("gs_dx".equals(Global.G_instArea)&&"INTERNET".equals(entry.getValue())){
			...
			if (paramName.indexOf(wanConnectionType) >= 0)
			{
				...
				if(entry.getValue().equals("PPPoE_Bridged"))
				{
					wanConnMap.remove(j);
					logger.warn("移除桥接通道，测速只走路由通道");
				}
				continue;
			}
		...
		for (Map.Entry<String, WanConnObj> entryConn : wanConnMap.entrySet())
		{
			...
			//增加判断，陕西的vlanid为空的就返回通道ijk值，其他省份根据Vlanid判断
			if(Vlanid.equals(tempWanConnObj111.getVlanid()) || ("sx_dx".equals(Global.G_instArea) && "".equals(tempWanConnObj111.getVlanid()))){
	...
	public static void main(String args[]){
		...
		String paramName= "InternetGatewayDevice.WANDevice.1.WANConnectionDevice.1.";
		System.out.println(wanConnPath.length());
		System.out.println(paramName.indexOf(".", wanConnPath.length()));
