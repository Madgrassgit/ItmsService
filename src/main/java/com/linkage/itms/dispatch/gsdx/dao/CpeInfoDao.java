...
package com.linkage.itms.dispatch.gsdx.dao;
...
public class CpeInfoDao
{
	...
	public boolean insertWan(String deviceId, String wanId, String accessType)
	{
		...
		if (DBOperation.executeUpdate(usBuilder.toString()) > 0)
		{
			...
			usBuilder.append(
					"insert into gw_wan(device_id, wan_id, access_type, gather_time) values('");
	...
	public Map<String, String> getDeviceVersion(String deviceId)
	{
		...
		sql.append(
				...
				.append("where 1=1 ").append("  and a.vendor_id = b.vendor_id ")
	...
	public Map<String, String> queryUserInfoNew(int userType, String username)
	{
		...
		switch (userType)
		{
			...
			// 用户宽带帐号
			case 1:
				strSQL = "select a.device_id,a.user_id,a.username as logic_id,a.device_serialnumber,a.spec_id,t1.complete_time from "
						+ table_customer
						+ " a  inner join tab_gw_device t1 on a.device_id = t1.device_id ,"
						+ table_serv_info
						+ " b where a.user_id = b.user_id and b.serv_type_id = 10 and b.username='"
						+ username + "'";
				break;
			// 电话号码
			case 2:
				strSQL = "select a.device_id,a.user_id,a.username as logic_id,a.device_serialnumber,a.spec_id,t1.complete_time from "
						+ table_customer
						+ " a  left join tab_gw_device t1 on a.device_id = t1.device_id,"
						+ table_voip
						+ " b where a.user_id = b.user_id and b.voip_username='"
						+ username + "'";
				break;
			// Device ID
			case 3:
			...
			// IP地址
			case 4:
				strSQL = "select a.device_id,a.user_id,a.username as logic_id,b.device_serialnumber,a.spec_id,b.complete_time from "
						+ table_customer
						+ " a, tab_gw_device b where a.device_id = b.device_id and b.loopback_ip='"
						+ username + "'";
				break;
			// SN
			case 5:
				...
				{
					devSubSn = username.substring(username.length() - 6,
							username.length());
					strSQL += " and t1.dev_sub_sn = '" + devSubSn + "'";
				}
				break;
		...
		ArrayList<HashMap<String, String>> userIdList = DBOperation
				.getRecords(new PrepareSQL(strSQL).getSQL());
		Map<String, String> userInfoMap = new HashMap<String, String>();
		if (null != userIdList && !userIdList.isEmpty() && null != userIdList.get(0))
		{
			Collections.sort(userIdList, new Comparator<Map<String, String>>()
			{

				public int compare(Map<String, String> o1, Map<String, String> o2)
				{
					String name1 = (String) o1.get("complete_time");// name1是从你list里面拿出来的一个
					String name2 = (String) o2.get("complete_time"); // name1是从你list里面拿出来的第二name
					return name2.compareTo(name1);
				}
			});
			userInfoMap.put("loid", userIdList.get(0).get("logic_id"));
			userInfoMap.put("device_serialnumber",
					userIdList.get(0).get("device_serialnumber"));
			...
			String servSQL = "select a.username as pppusename,a.wan_type,a.serv_type_id,a.passwd,a.open_status,a.bind_port,b.voip_username,b.voip_port,b.parm_stat"
					...
					+ userId;
			ArrayList<HashMap<String, String>> userServList = DBOperation
					.getRecords(new PrepareSQL(servSQL).getSQL());
			if (null != userServList && !userServList.isEmpty()
					&& null != userServList.get(0))
			{
				String netName = "";
				String netPassword = "";
				String netStatus = "";
				...
				for (HashMap<String, String> userinfoMap : userServList)
				{
					// 宽带
					if ("10".equals(
							StringUtil.getStringValue(userinfoMap, "serv_type_id"))
							&& StringUtil.isEmpty(netName))
					{
							...
							StringUtil.getStringValue(userinfoMap, "serv_type_id")))
					{
						...
						{
							iptvName = StringUtil.getStringValue(userinfoMap,
									"pppusename");
							iptvStatus = StringUtil.getStringValue(userinfoMap,
									"open_status");
							iptvPort = StringUtil.getStringValue(userinfoMap,
									"bind_port");
						}
						else
						{
							iptvName = iptvName + "|" + StringUtil
									.getStringValue(userinfoMap, "pppusename");
							iptvStatus = iptvStatus + "|" + StringUtil
									.getStringValue(userinfoMap, "open_status");
							iptvPort = iptvPort
									+ StringUtil.getStringValue(userinfoMap, "bind_port");
						}
					}
					// 语音
					if ("14".equals(
							StringUtil.getStringValue(userinfoMap, "serv_type_id")))
					{
						...
						{
							voipName = StringUtil.getStringValue(userinfoMap,
									"voip_username");
							voipPort = StringUtil.getStringValue(userinfoMap,
									"voip_port");
							voipStatus = StringUtil.getStringValue(userinfoMap,
									"parm_stat");
						}
						else
						{
							voipName = voipName + "|" + StringUtil
									.getStringValue(userinfoMap, "voip_username");
							voipPort = voipPort + "|"
									+ StringUtil.getStringValue(userinfoMap, "voip_port");
							voipStatus = voipStatus + "|"
									+ StringUtil.getStringValue(userinfoMap, "parm_stat");
						}
					}
				}
			...
			String deviceSql = "select * from tab_gw_device where device_id='"
					+ StringUtil.getStringValue(userInfoMap, "device_id") + "'";
			if (3 == DBUtil.GetDB())
			{
				deviceSql = "select loopback_ip from tab_gw_device where device_id='"
						+ StringUtil.getStringValue(userInfoMap, "device_id") + "'";
			}
			ArrayList<HashMap<String, String>> deviceList = DBOperation
					.getRecords(new PrepareSQL(deviceSql).getSQL());
			if (null != deviceList && !deviceList.isEmpty() && null != deviceList.get(0))
			{
				userInfoMap.put("loopback_ip",
						StringUtil.getStringValue(deviceList.get(0), "loopback_ip"));
			}
			else
			{
				userInfoMap.put("loopback_ip", "");
	...
	public Map<String, String> queryUserInfo(int userType, String username)
	{
		...
		switch (userType)
		{
			...
			case 0:
				...
				{
					strSQL += " username = '" + username
							+ "' order by ISNULL(t1.complete_time), t1.complete_time desc";
				}
				else
				{
					strSQL += " username = '" + username
							+ "' order by t1.complete_time desc nulls last";
				}
				break;
			// 用户宽带帐号
			case 1:
				strSQL = "select a.device_id,a.user_id,a.username as logic_id,a.oui,a.device_serialnumber,a.spec_id,t1.complete_time from "
						+ table_customer
						+ " a  left join tab_gw_device t1 on a.device_id = t1.device_id ,"
						+ table_serv_info
						+ " b where a.user_id = b.user_id and b.serv_type_id = 10 and b.username='"
						+ username + "' order by t1.complete_time desc nulls last";
				if (3 == DBUtil.GetDB())
				{
					strSQL = "select a.device_id,a.user_id,a.username as logic_id,a.oui,a.device_serialnumber,a.spec_id,t1.complete_time from "
							+ table_customer
			...
			case 3:
				...
				{
					strSQL += " and t1.device_serialnumber like '%" + devSubSn + "%'";
				}
				break;
			// IP地址
			case 4:
			...
			case 5:
				...
				if (username.length() > 6)
				{
					strSQL += " and t1.dev_sub_sn = '"
							+ username.substring(username.length() - 6, username.length())
							+ "'";
				}
				break;
			default:
	...
	public Map<String, String> getDeviceTypeInfo(String vendor_id, String device_model_id,
			String fileName)
	{
		String strSQL = "select devicetype_id from gw_device_model where vendor_id = '"
				+ vendor_id + "' and  device_model_id = '" + device_model_id
				+ "' and softwareversion = '" + fileName + "'";
		PrepareSQL psql = new PrepareSQL(strSQL);
		return DBOperation.getRecord(psql.getSQL());
	}
	...
	public Boolean strategySQL(StrategyOBJ obj)
	{
				...
				&& 1 == Global.IS_LARGE_VALUE)
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				if (((i + 1) * Global.SPIT_NUM_LARGE_VAL + subStrCount) < sheet_param
						.length())
				{
					sheet_para_val = sheet_param.substring(
							i * Global.SPIT_NUM_LARGE_VAL - subStrCount,
							(i + 1) * Global.SPIT_NUM_LARGE_VAL);
				}
				else
				{
					sheet_para_val = sheet_param
							.substring(i * Global.SPIT_NUM_LARGE_VAL - subStrCount);
				}
				// 结算结尾空格数
				subStrCount = sheet_para_val.length()
						- ("A" + sheet_para_val).trim().length() + 1;
				sheet_para_val = sheet_para_val.substring(0,
						sheet_para_val.length() - subStrCount);
				sql3.append(
		...
		sql1.append(
				"redo,id,acc_oid,time,type,device_id,oui,device_serialnumber,username,sheet_para,service_id,task_id,order_id,sheet_type, temp_id, is_last_one,priority,client_id");
		sql1.append(") values (");
	...
	public int doBatch(List<String> sqlList)
	{
		...
		{
			result = DBOperation.executeUpdate((ArrayList<String>) sqlList);
		}
		return result;
	}
	...
	public Map<String, String> getSoftResultInfo(String deviceId)
	{
		String sql = "select a.status,a.device_id,a.result_id,c.fault_desc from gw_serv_strategy_soft a,tab_gw_device b,tab_cpe_faultcode c "
				+ "where a.result_id = c.fault_code and a.service_id= 5 and a.is_last_one=1  where a.device_id = '"
				+ deviceId + "'";
	...
	public Map<String, String> getServResultInfo(String deviceId)
	{
		...
		prepareSQL.append(
				"select end_time from gw_serv_strategy_serv where result_id = 1 and is_last_one=1  and device_id = ? ");
		prepareSQL.setString(1, deviceId);
		return DBOperation.getRecord(prepareSQL.getSQL());
	}
	...
	public String getAccessStyleType(String deviceId)
	{
		...
		{
			logger.warn("[{}]采用用户表中的上行方式：{}", deviceId, type);
			return type;
		}
		else
		{
	...
	{
		String sql = "select access_style_id from tab_hgwcustomer where device_id='"
				+ deviceId + "'";
		Map<String, String> numMap = DBOperation.getRecord(sql);
		if (null != numMap)
		{
			return numMap.get("access_style_id");
		}
		return null;
	}
	...
	public Map<String, String> getDevStatus(String userId)
	{
		...
		String tableCustomer = "tab_hgwcustomer";
		if ("BBMS".equals(Global.SYSTEM_NAME))
		{
			tableCustomer = "tab_egwcustomer";
		}
		String strSQL = "select a.user_id,b.device_id,b.complete_time,b.vendor_id,b.device_model_id,b.oui,b.device_serialnumber,b.devicetype_id,"
				+ " c.online_status, c.last_time,a.city_id" + " from " + tableCustomer
				+ " a left join tab_gw_device b on a.device_id=b.device_id"
				+ " left join gw_devicestatus c on  a.device_id=c.device_id "
				+ " where a.user_state in ('1','2')" + " and a.user_id=" + userId;
		logger.info(strSQL);
	...
	{
		String sql = "select serv_type_id from HGWCUST_SERV_INFO  where user_id="
				+ user_id;
		Map<String, String> numMap = DBOperation.getRecord(sql);
		if (null != numMap)
		{
			return numMap.get("serv_type_id");
		}
		return null;
	}
	...
	public int getServOpenStatus(String user_id)
	{
		try
		{
			String sql = "update HGWCUST_SERV_INFO  set open_status  = 0 where user_id = "
					+ user_id + " and serv_type_id = 14";
			logger.info(sql);
	...
	public Map<String, String> getDeviceVersionAHLT(String deviceId)
	{
		StringBuilder sql = new StringBuilder();
		sql.append("select d.device_id, " + "       a.access_style_relay_id,"
				+ "       c.vendor_add,c.vendor_name," + "       b.device_model,"
				+ "       a.hardwareversion," + "       a.softwareversion,"
				+ "       a.rela_dev_type_id," + "       a.is_qos"
				+ "  from tab_gw_device       d," + "       tab_devicetype_info a,"
				+ "       gw_device_model     b," + "       tab_vendor          c"
				+ " where d.devicetype_id = a.devicetype_id"
				+ "   and a.device_model_id = b.device_model_id"
				+ "   and a.vendor_id = c.vendor_id" + "   and d.device_id = '" + deviceId
				+ "'");
		PrepareSQL psql = new PrepareSQL();
		psql.setSQL(sql.toString());
		return DBOperation.getRecord(psql.getSQL());
	...
	public String getServList(String userId)
	{
		StringBuilder returnStr = new StringBuilder();
		String sql = "select serv_type_id,wan_type from hgwcust_serv_info  where user_id="
				+ userId;
		ArrayList<HashMap<String, String>> numMap = DBOperation.getRecords(sql);
		if (numMap != null && !numMap.isEmpty())
		{
			for (int i = 0; i < numMap.size(); i++)
			{
				String servTypeId = numMap.get(i).get("serv_type_id");
				String wanType = numMap.get(i).get("wan_type");
				if ("10".equals(servTypeId))
				{
					if ("1".equals(wanType))
					{
						returnStr.append("wband_bridge");
					}
					else if ("2".equals(wanType))
					{
						returnStr.append("wband_route");
					}
					else
					{
						returnStr.append("wband");
					}
				}
				else if ("11".equals(servTypeId))
				{
					returnStr.append("iptv");
				}
				else if ("14".equals(servTypeId))
				{
					returnStr.append("voip");
				}
				if (i != numMap.size() - 1)
				{
					returnStr.append(",");
