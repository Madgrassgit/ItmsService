...
public class GetCpeFlowXML extends BaseDealXML {

	public GetCpeFlowXML(String methodName) {
		super(methodName);
	}

	private static Logger logger = LoggerFactory.getLogger(GetCpeFlowXML.class);
	...
	public Document getXML(String inXml) {
		...
		try {
			...
			/**
			 * 0：逻辑ID，即激活码 1：宽带帐号，即Order结构中的ad_account字段。 2：Device ID(OUI-SN)。3：Device ID(OUI-SN)
			 */
