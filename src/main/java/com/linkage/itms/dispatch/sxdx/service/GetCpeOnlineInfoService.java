...
import com.linkage.itms.dispatch.sxdx.beanObj.CpeBasicInfo;
import com.linkage.itms.dispatch.sxdx.beanObj.Para;
import com.linkage.itms.dispatch.sxdx.beanObj.ServiceInfo;
		...
		if(null == baseList || baseList.size()==0){
			...
			if(!StringUtil.isEmpty(StringUtil.getStringValue(baseMap, "user_id"))){
				...
				if(null != servList && !servList.isEmpty()){
					logger.warn(methodName+"["+dealXML.getOpId()+"]查询到业务");
			...
			{
				logger.warn(methodName+"["+dealXML.getOpId()+"]设备在线，批量采集节点失败，device_id={},返回-1", deviceId);
				return -1;
			}
			else
			{
				logger.warn(methodName+"["+dealXML.getOpId()+"]设备在线，批量采集节点成功，device_id={}", deviceId);
				if(null != iptv){
