...
public class TestSpeedChecker4SDLT extends BaseChecker{

	
	private static Logger logger = LoggerFactory.getLogger(TestSpeedChecker4SDLT.class);
	 ...
	 *            接口调用入参，xml字符串
	 */
	public TestSpeedChecker4SDLT(String inXml) {
		this.callXml = inXml;
	}
	/**
	 * 检查接口调用字符串的合法性
	...
	public boolean check()
	{
		...
		else if(StringUtil.IsEmpty(testType)){
			result = 1;
			resultDesc = "testType为空";
			return false;
		}
		result = 0;
		resultDesc = "成功";
	...
	public String getReturnXml()
	{
		...
		root.addElement("DevSn").addText(StringUtil.IsEmpty(devSn)?"--":devSn);
		root.addElement("Aspeed").addText(StringUtil.IsEmpty(Aspeed)?"--":Aspeed );
		root.addElement("maxspeed").addText(StringUtil.IsEmpty(maxspeed)?"--":maxspeed);
		root.addElement("cityName").addText(StringUtil.IsEmpty(cityName)?"--":cityName);
		root.addElement("deviceModel").addText(StringUtil.IsEmpty(deviceModel)?"--":deviceModel);
		root.addElement("deviceVendor").addText(StringUtil.IsEmpty(deviceVendor)?"--":deviceVendor);
		root.addElement("softwareversion").addText(StringUtil.IsEmpty(softwareversion)?"--":softwareversion);
		root.addElement("loid").addText(StringUtil.IsEmpty(loid)?"--":loid);
		root.addElement("userName").addText(StringUtil.IsEmpty(userName)?"--":userName);
