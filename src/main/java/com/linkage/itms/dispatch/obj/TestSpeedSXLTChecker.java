 ...
 *@修改人和其它信息
 */
public class TestSpeedSXLTChecker extends BaseChecker{

private static final Logger logger = LoggerFactory.getLogger(TestSpeedSXLTChecker.class);
	...
	// 仿真宽带账号
	private String eupppoename = null;

	// 仿真密码
	private String eupassword = null;
	...
	private String SpeedTest_DiagnosticsState = "";
	private String SpeedTest_downloadURL = "";
	private String SpeedTest_testMode = "";
	private String SpeedTest_reportURL = "";
	private String SpeedTest_testURL = "";
	//用户的宽带账号
	private String pppoeUserName=null;

	public TestSpeedSXLTChecker(String inParam){
	...
	public boolean check(){
		...
		logger.warn("TestSpeedChecker==>check()");
		...
		try {
			...
			logger.warn("userInfoType:[{}]",userInfoType);
			...
			eupppoename = param.elementTextTrim("eupppoename");
			eupassword = param.elementTextTrim("eupassword");*/
			
			
			document = reader.read(new StringReader(inParam));
			Element root = document.getRootElement();
			Element param = root.element("Param");
			cmdId = StringUtil.getStringValue(param.elementTextTrim("op_id"));
			//source = StringUtil.getStringValue(param.elementTextTrim("source"));
			backgroundsize = StringUtil.getStringValue(param.elementTextTrim("SpeedTest_backgroundsize"));
			userInfoType = StringUtil.getIntegerValue(param.elementTextTrim("type"));
			netAccount = StringUtil.getStringValue(param.elementTextTrim("index"));
			userInfo = netAccount;
			SpeedTest_DiagnosticsState = StringUtil.getStringValue(param.elementTextTrim("SpeedTest_DiagnosticsState"));
			SpeedTest_downloadURL = StringUtil.getStringValue(param.elementTextTrim("SpeedTest_downloadURL"));
			SpeedTest_testMode = StringUtil.getStringValue(param.elementTextTrim("SpeedTest_testMode"));
			SpeedTest_reportURL = StringUtil.getStringValue(param.elementTextTrim("SpeedTest_reportURL"));
			SpeedTest_testURL = StringUtil.getStringValue(param.elementTextTrim("SpeedTest_testURL"));
			pppoeUserName = StringUtil.getStringValue(param.elementTextTrim("SpeedTest_pppoeName"));
			eupppoename = StringUtil.getStringValue(param.elementTextTrim("SpeedTest_eupppoename"));
			eupassword = StringUtil.getStringValue(param.elementTextTrim("SpeedTest_eupassword"));
			//wanPassageWay = StringUtil.getStringValue(param.elementTextTrim("WanPassageWay"));
			
			
		} catch (Exception e) {
			logger.error("inParam format is err,mesg({})", e);
	...
	public String getReturnXml(){
		...
		/*if(0 == result){
			...
			//RESP_TYPE 测速类型GZ:公众 HGU光猫测速
			root.addElement("RESP_TYPE").addText("HGU");
		}*/
	...
	private boolean otherParamCheck(){
		...
		if(StringUtil.IsEmpty(SpeedTest_reportURL)){
			result = 8;
			resultDesc = "上报URL非法";
			return false;
		}
		if(StringUtil.IsEmpty(SpeedTest_testURL)){
			result = 9;
			resultDesc = "测速URL非法";
			return false;
		}
	...
	public String getEupppoename() {
		return eupppoename;
	}

	public void setEupppoename(String eupppoename) {
		this.eupppoename = eupppoename;
	}
	...
	public String getSpeedTest_DiagnosticsState()
	{
		return SpeedTest_DiagnosticsState;
	}

	
	public void setSpeedTest_DiagnosticsState(String speedTest_DiagnosticsState)
	{
		SpeedTest_DiagnosticsState = speedTest_DiagnosticsState;
	}
	...
	{
		SpeedTest_downloadURL = speedTest_downloadURL;
	}
	...
	{
		SpeedTest_testMode = speedTest_testMode;
	}
	...
	{
		SpeedTest_reportURL = speedTest_reportURL;
	}

	
	public String getSpeedTest_testURL()
	{
		return SpeedTest_testURL;
	}

	
	public void setSpeedTest_testURL(String speedTest_testURL)
	{
		SpeedTest_testURL = speedTest_testURL;
	}
