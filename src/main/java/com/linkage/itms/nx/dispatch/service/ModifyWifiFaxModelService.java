...
import com.linkage.itms.nx.dispatch.obj.ModifyWifiFaxModelChecker;
 ...
 * @copyright 
 */
public class ModifyWifiFaxModelService implements NxIService
{
	/** 日志记录 */
	private static Logger logger = LoggerFactory.getLogger(ModifyWifiFaxModelService.class);
	@Override
	public String work(String inXml)
	{
		logger.warn("ModifyWifiFaxModelService, inParam:({})", inXml);
		ModifyWifiFaxModelChecker checker = new ModifyWifiFaxModelChecker(inXml);
		// 验证入参格式是否正确
		if (false == checker.check())
		{
			logger.error("servicename[ModifyWifiFaxModelService],cmdId[{}],验证未通过，返回：{}",
					new Object[] { checker.getCmdId(), checker.getReturnXml() });
			logger.warn("ModifyWifiFaxModelService,cmdId[{}],return=({})", checker.getCmdId(),
...
//		}
		...
		else if (1 == flag)
		{
			...
			String faxModelPath = "InternetGatewayDevice.Services.VoiceService.1.VoiceProfile.1.FaxT38.Enable";
			String faxModelValue = checker.getFaxPattern();
			ArrayList<ParameValueOBJ> objLlist = corba.getValue(deviceId, faxModelPath);
//			List<String> iList = corba.getIList(deviceId, channelPath);
			...
			else
			{
				...
				pvOBJ.setName(faxModelPath);
				pvOBJ.setValue(faxModelValue);
				...
				if (0 == retResult || 1 == retResult)
				{
					...
					new RecordLogDAO().recordDispatchLog(checker, checker.getUserInfo(),
							"ModifyWifiFaxModelService");
					logger.warn(
							"servicename[ModifyWifiFaxModelService]cmdId[{}]userinfo[{}]处理结束，返回响应信息:{}",
