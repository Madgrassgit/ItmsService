...
import com.linkage.itms.nx.dispatch.obj.ModifyWifiSendPowerChecker;
 ...
 * @copyright 
 */
public class ModifyWifiSendPowerService implements NxIService
{
	/** 日志记录 */
	private static Logger logger = LoggerFactory.getLogger(ModifyWifiSendPowerService.class);
	@Override
	public String work(String inXml)
	{
		logger.warn("ModifyWifiSendPowerService, inParam:({})", inXml);
		ModifyWifiSendPowerChecker checker = new ModifyWifiSendPowerChecker(inXml);
		// 验证入参格式是否正确
		if (false == checker.check())
		{
			logger.error("servicename[ModifyWifiSendPowerService],cmdId[{}],验证未通过，返回：{}",
					new Object[] { checker.getCmdId(), checker.getReturnXml() });
			logger.warn("ModifyWifiSendPowerService,cmdId[{}],return=({})", checker.getCmdId(),
...
//		}
		...
		else if (1 == flag)
		{
			...
			String powerLevelPath = "InternetGatewayDevice.LANDevice.1.WLANConfiguration.1.X_CT-COM_Powerlevel";
			String powerLevelValue = checker.getPowerlevel();
			String powerValue = "";
			String powerValuePathString = "InternetGatewayDevice.LANDevice.1.WLANConfiguration.1.X_CT-COM_PowerValue";
			ArrayList<ParameValueOBJ> objLlist = corba.getValue(deviceId, powerLevelPath);
			ArrayList<ParameValueOBJ> objLlist1 = corba.getValue(deviceId, powerValuePathString);
			if(null != objLlist1 && !objLlist1.isEmpty())
			{
				powerValue = objLlist1.get(0).getValue();
				checker.setPowerValue(powerValue);
			}
...
//			List<String> iList = corba.getIList(deviceId, channelPath);
			...
			else
			{
				...
				pvOBJ.setName(powerLevelPath);
				pvOBJ.setValue(powerLevelValue);
				...
				if (0 == retResult || 1 == retResult)
				{
					...
					logger.warn(
							"servicename[ModifyWifiSendPowerService]cmdId[{}]userinfo[{}]处理结束，返回响应信息:{}",
