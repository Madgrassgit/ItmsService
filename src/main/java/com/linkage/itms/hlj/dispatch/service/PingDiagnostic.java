...
import com.linkage.itms.hlj.dispatch.obj.PingDiagnosticChecker;
...
public class PingDiagnostic implements HljIService
{
	...
	{
		logger.warn("PingDiagnostic==>jsonString({})", jsonString);
		PingDiagnosticChecker checker = new PingDiagnosticChecker(jsonString);
		if (false == checker.check())
		{
			logger.warn("ping测试接口，入参验证失败，QueryNum=[{}]",
					new Object[] { checker.getQueryNum() });
			logger.warn("PingDiagnostic==>retParam={}", checker.getReturnXml());
			return checker.getReturnXml();
		}
		String deviceId = "";
		QueryDeviceIdDAO qdDao = new QueryDeviceIdDAO();
		List<HashMap<String, String>> userMap = null;
		if (checker.getQueryType() == 1)
		{
		...
		{
			userMap = qdDao.queryUserByMac(checker.getQueryNum());
		}
		else
		{
		}
		...
		return PingList("1", deviceId, checker);
	}
	...
	public String PingList(String gw_type, String device_id, PingDiagnosticChecker checker)
	{
		...
		else
		{
			...
			wanConnIds = dao.getWanInterface(device_id, vlanId);
			if (wanConnIds == null || wanConnIds.isEmpty())
			{
				logger.warn("servicename[PingDiagnostic]QueryNum[{}]设备未获取到wan口",
						new Object[] { checker.getQueryNum() });
				checker.setResult(6);
			...
			else
			{
				...
				{
					logger.warn(
							"servicename[PingDiagnostic]QueryNum[{}]设备获取到wan口sessType值不对",
							new Object[] { checker.getQueryNum() });
					checker.setResult(6);
					checker.setResultDesc("设备未知错误");
					logger.warn("PingDiagnostic==>ReturnXml:" + checker.getReturnXml());
		...
		logger.warn("servicename[PingDiagnostic]QueryNum[{}]设备获取到wan口[{}]",
		...
		anyObject = new AnyObject();
		anyObject.para_value = checker.getDestIp();
		anyObject.para_type_id = "1";
		...
		anyObject = new AnyObject();
		anyObject.para_value = checker.getPackCount();
		...
		anyObject = new AnyObject();
		anyObject.para_value = checker.getPackSize();
		anyObject.para_type_id = "3";
		...
		else
		{
			...
			else
			{
				...
				else
				{
					...
					long PackageCount = StringUtil.getIntegerValue(checker.getPackCount());
					String packetLossRate = percent(FailureCount, PackageCount);
//					String iPOrDomainName = checker.getDestIp() ;
					String sourceIp = "";
					sourceIp = StringUtil.getStringValue(wanConnIds
							.get("ip"));
					...
					try
					{
						...
						if ("NULL".equals(sourceIp)||"null".equals(sourceIp)||sourceIp.isEmpty()){
							jo.put("sourceIP","");
						}else{
							jo.put("sourceIP",sourceIp);
						}

						jo.put("resultList", resultList);
						resultList.put("testNum", "");//置空
						resultList.put("sendPacks", checker.getPackCount());
						resultList.put("receiPacks", succesNum);
						resultList.put("lossRate", packetLossRate);
						resultList.put("timeLag", "");//置空
					}
					catch (JSONException e)
					{
						e.printStackTrace();
					}
					logger.warn("PingDiagnostic==>return:" + jo.toString());
					return jo.toString();
