...
public class BindIptvStbMacChecker extends BaseChecker{
	
	private static final Logger logger = LoggerFactory.getLogger(BindIptvStbMacChecker.class);
	
	private String inParam = null;

	public BindIptvStbMacChecker(String inParam){
	...
	public boolean check(){
		...
		logger.warn("BindIptvStbMacChecker==>check()==>检查参数合法性");
		...
		try {
			 ...
			 * 需要绑定的业务账号
			 */
			userAccount = param.elementTextTrim("UserAccount");
		...
		if (false == baseCheck() || false == searchTypeCheck()
				|| false == searchInfoCheck() || false == userAccountCheck()) { 
			logger.warn("BindIptvStbMacChecker==>参数不合法");
			return false;
		}
