...
public class AxisServlet extends AxisServletBase {
    ...
    public void doPost(HttpServletRequest req, HttpServletResponse res) throws
        ...
        try {
            ...
            processAxisFault(fault);
            configureResponseFromAxisFault(res, fault);
            responseMsg = msgContext == null ? null : msgContext.getResponseMessage();
            if (responseMsg == null) {
                responseMsg = new Message(fault);
                ((org.apache.axis.SOAPPart) responseMsg.getSOAPPart()).
                        getMessage().setMessageContext(msgContext);
        ...
        if (responseMsg != null) {
            ...
            String responseEncoding = msgContext == null ? null : ((String) msgContext.getProperty(
        ...
        if (tlog.isDebugEnabled()) {
            ...
            tlog.debug("axisServlet.doPost: " + soapAction +
                       ...
                       " post=" + (t3 - t2) +
                       " send=" + (t4 - t3) +
                       " " + (msgContext == null ? "" : msgContext.getTargetService()) + "." +
                       ((msgContext == null || msgContext.getOperation() == null) ?
    ...
    private String getSoapAction(HttpServletRequest req) throws AxisFault {
        ...
        if (null != soapAction && soapAction.startsWith("\"") && soapAction.endsWith("\"")
            && soapAction.length() >= 2) {
            int end = soapAction.length() - 1;
            soapAction = soapAction.substring(1, end);
        }

        if (null == soapAction || soapAction.length() == 0) {
